...
<template>
  <div
    class="fixed bottom-[0.6em] end-[1em] z-[90] !hidden rotate-90 transition-transform duration-300 after:absolute after:end-0 after:top-0 after:block after:size-12 after:rounded-full after:bg-primary-600 after:shadow-lg after:shadow-primary-500/50 after:transition-transform after:duration-300 after:content-[''] dark:after:shadow-muted-800/10"
    :class="[
  ...
  >
    <button
      type="button"
      class="relative z-30 flex size-12 -rotate-90 items-center justify-center rounded-full bg-primary-500 text-white shadow-lg shadow-primary-500/50 dark:shadow-muted-800/10"
      @click="isOpened = !isOpened"
    >
      <span
        class="relative block size-3 transition-all duration-300"
        :class="isOpened ? 'top-0 scale-90' : '-top-0.5'"
      >
        <span
          class="absolute block h-0.5 w-full bg-muted-50 transition-all duration-300"
          :class="isOpened ? 'top-1 -rotate-45' : 'top-0.5'"
        />
        <span
          class="absolute top-1/2 block h-0.5 w-full bg-muted-50 transition-all duration-300"
          :class="isOpened ? 'translate-x-4 opacity-0' : ''"
        />
        <span
          class="absolute block h-0.5 w-full bg-muted-50 transition-all duration-300"
          :class="isOpened ? 'bottom-1.5 rotate-45' : 'bottom-0'"
        />
      </span>
    </button>
    ...
    <div>
      <template v-for="(tool, idx) in tools">
        ...
        >
          <component :is="resolveComponentOrNative(tool.component)" v-bind="tool.props" />
        </div>
      </template>
    </div>
  </div>
