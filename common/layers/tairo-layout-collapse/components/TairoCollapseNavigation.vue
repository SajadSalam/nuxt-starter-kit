...
<template>
  <div
    class="border-muted-700 fixed start-0 top-0 z-[60] flex h-full flex-col border-r bg-muted-900 transition-all duration-300"
    :class="[
      ...
      <ul v-if="startMenuItems?.length" class="space-y-2">
        ...
        <li v-for="(item, index) in startMenuItems" :key="index">
          ...
          <div
            ...
            class="border-muted-700 my-3 h-px w-full border-t"
          />
          <button
            v-else
            class="nui-focus text-muted-400 hover:bg-muted-700/60 hover:text-muted-200 flex w-full cursor-pointer items-center gap-4 rounded-lg py-3 transition-colors duration-300"
            :class="!isOpen ? 'px-1 justify-center' : 'px-4'"
            @click="item.click"
          >
            <Icon :name="item.icon.name" :class="item.icon.class" />
