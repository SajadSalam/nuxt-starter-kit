...
<template>
  <div class="group">
    <button
      ref="buttonRef"
      class="nui-focus text-muted-400 hover:bg-muted-700/60 hover:text-muted-200 flex w-full cursor-pointer items-center rounded-lg py-3 transition-colors duration-300"
    ...
    <ul
      v-if="props.expanded"
      class="border-muted-700 relative block ps-4"
      :class="{
        'max-h-0 overflow-hidden opacity-0 group-focus-within:max-h-max group-focus-within:overflow-visible group-focus-within:opacity-100':
          !isOpen,
        'after:border-muted-700 max-h-max opacity-100': isOpen,
      }"
    >
      <li
        v-for="child in props.item.children"
        :key="child.to"
        class="border-muted-700 ms-2 border-s-2 first:mt-2"
      >
        <NuxtLink
          :to="child.to"
          exact-active-class="!border-primary-500 !text-primary-400"
          class="nui-focus text-muted-400 hover:text-muted-200 relative -start-0.5 flex cursor-pointer items-center gap-2 border-s-2 border-transparent py-2 ps-4 transition-colors duration-300"
        >
          <Icon
